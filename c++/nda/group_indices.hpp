...
namespace nda {
  ...
  namespace detail {
    ...
    // Given an original stride order and a partition of the indices, determine the new stride
    // order of the grouped nda::idx_map.
    template <size_t R, size_t... Rs>
    constexpr std::array<int, sizeof...(Rs)> stride_order_of_grouped_idx_map(std::array<int, R> const &stride_order,
      ...
      // check if the indices/dimensions in a group are consecutive in memory and find the slowest
      // varying index within the group
      ...
      // rank each group by its slowest varying index, i.e. the slowest group gets rank 0 and the
      // fastest rank Ngrps-1
  ...
  /**
  * @brief Given an nda::idx_map and a partition of its indices, return a new nda::idx_map
  * with the grouped indices.
  *
  * @details It is used in nda::group_indices_view: `group_indices_view(A, idx_group<i,j,...>,
  * idx_group<k,l,...>, ...)`. The resulting index map is of rank `sizeof...(IdxGrps)` with
  * merged indices, one for each group.
  *
  * Preconditions:
  * - The given index groups correspond to a partition of the set `{0, 1, ..., Rank - 1}`.
  * - In each group the indices are consecutive in memory .
  *
  * Indices can only be grouped together if they are consecutive in the memory layout.
  *
  * For example, let's look at the following strider order: S = (2, 1, 0). That means that
  * dimension 0 is the fastest varying index, then dimension 1 and dimension 2 is slowest.
  * We say that dimensions 0 and 1 are consecutive in memory.
  * The same goes for dimensions 2 and 1. However, dimensions 0 and 2 are not consecutive
  * in memory.
  *
  * To check if a group of indices are consecutive in memory, we look at the inverse permutation
  * of the stride order. The stride order S is a permutation which maps an integer j to a
  * dimension/index i of the array such that S[j] = i is the (j+1)-th slowest varying index, i.e.
  * S[0] is the slowest index and S[R-1] is the fastest. The inverse permutation, M = S^{-1},
  * therefore tells us how fast a given index varies, e.g. M[i] = j means that index i varies the
  * (j+1)-th slowest.
  *
