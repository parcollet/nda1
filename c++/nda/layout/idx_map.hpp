...
namespace nda {
   ...
   *
   * @details It stores the shape of the array, i.e. the length of each dimension, and the strides
   * of each dimension. The stride of dimension `i` is the number of elements to skip in memory when
   * the index of dimension `i` is incremented by one. For example:
   ...
   *
   * The template parameters `StaticExtents` and `StrideOrder` are encoded as `uint64_t`. They can
   * be decoded to a `std::array<int, Rank>` using the nda::decode function. The encoding limits
   * the number of dimensions, i.e. the rank, to 16.
   *
   * The static extent array specifies the length of each dimension at compile-time. A zero value
   * means that the length in this dimension is dynamic and will be specified at runtime. Note that
   * static lengths cannot exceed 16 (due to the encoding). For example:
   * - `StaticExtents = nda::encode(std::array<int, Rank>{0, 10, 0, 5})` corresponds to a 4D array
   * with dynamic extents in dimension 0 and 2, and static extents of length 10 and 5 in dimension
   * 1 and 3, respectively.
   *
   * The stride order specifies the order in which the dimensions are stored in memory. In its array
   * form, it can be any permutation of the integer values from `0` to `Rank - 1`. The slowest varying
   * dimension is the first element of the array and the fastest varying dimension is the last element
   ...
   *
   * `LayoutProp` is an enum flag that can be used to make further compile-time guarantees about the
   * layout of the data in memory (see nda::layout_prop_e).
   *
   * @tparam Rank Number of dimensions.
  ...
  class idx_map {
    ...
    /**
     * @brief Alias template to check if type `T` can be used to either access a single element
     * or a slice of elements.
     */
    template <typename T>
     ...
     * @return Zero if it has at least one dynamic dimension, otherwise the product of the
     * static extents.
     */
     ...
     *
     * @details The data is contiguous in memory if the size of the map is equal to the number
     * of memory locations spanned by the map, i.e. the product of the largest stride times the
     ...
     *
     * @details The data is strided in memory with a constant stride if the size of the map times
     * the stride of the fastest dimension with an extent bigger than 1 is equal to the number of
     * memory locations spanned by the map, i.e. the product of the largest stride times the length
     ...
     *
     * @details For purely static maps, the shape is set to the static extents and the strides
     ...
     *
     * @details The missing extents are taken from the static extents, i.e. if a static
     * extent is zero, it is replaced by the corresponding dynamic extent.
     *
     * @param shape Array with the dynamic extents only.
     */
    ...
    // Is the smallest stride equal to one, i.e. is the fastest varying dimension contiguous
    // in memory?
    static constexpr bool smallest_stride_is_one = has_smallest_stride_is_one(layout_prop);

    // Implementation of the function call operator that takes a multi-dimensional index and
    // maps it to a linear index.
     ...
     *
     * @details All arguments are either convertible to type `long` or are of type nda::ellipsis.
     * The number of non-ellipsis arguments must be equal to the rank of the map and there must
     * be at most one nda::ellipsis. If an nda::ellipsis is present, it is skipped and does not
     * influence the result.
     *
     * Let \f$ (a_0, ..., a_{n-1}) \f$ be the given multi-dimensional index and let \f$ (s_0, ...,
     * s_{n-1}) \f$ be the strides of the index map. Then the equation that maps the multi-dimensional
     * index to the corresponding linear index \f$ f \f$ is as follows:
     * \f[
     ...
     *
     * @details Let \f$ f \f$ be the given linear index and let \f$ (s_0, ..., s_{n-1}) \f$
     * be the strides of the index map. Then the corresponding multi-dimensional index
     * \f$ (a_0, ..., a_{n-1}) \f$  satisfies the following equation:
     * \f[
     *   f = \sum_{i=0}^{n-1} a_i * s_i \; .
     * \f]
     *
     * Furthermore, if \f$ (p_0, ..., p_{n-1}) \f$ is the stride order of the map, let us define
     * the residues \f$ r_{p_i} \f$ as
     * \f[
     *   r_{p_i} = sum_{j=i}^{n-1} s_{p_j} * a_{p_j} \; ,
     * \f]
     * with the property that \f$ s_{p_i} /leq r_{p_i} < s_{p_{i-1}} \f$ for \f$ i = 1, ..., n-1 \f$
     * and \f$ s_{p_0} /leq r_{p_0} = f \f$. We can use this property to first calculate the residues
     * and then recover the multi-dimensional index.
     *
     ...
     *
     * @tparam Args Types of the arguments.
     * @param args Multi-dimensional index consisting of `long`, nda::range, nda::range::all_t or
     * nda::ellipsis objects.
     * @return A std::pair containing the offset in memory, i.e. the flat index of the first element
     * of the slice and the new nda::idx_map.
     */
    template <typename... Args>
    ...
    /**
     * @brief Create a new map by permuting the indices/dimensions of the current map with
     * a given permutation.
     *
     * @details Let `A` be the current and `A'` the new, permuted index map. `P` is the given
     * permutation. We define the permuted nda::idx_map `A'` to be the one with the following
     * properties:
     * - `A'(i_0,...,i_{n-1}) = A(i_{P[0]},...,i_{P[n-1]})`
     * - `A'.lengths()[k] == A.lengths()[P^{-1}[k]]`
     * - `A'.strides()[k] == A.strides()[P^{-1}[k]]`
     * - The stride order of `A'` is the composition of `P` and the stride order of `A` (note that
